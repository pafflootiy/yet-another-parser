...
def save_product(page_link, dir_name):
    ...
    product_breadcrumbs = response.html.find('.top_nav a')
    ...
    for breadcrumb in product_breadcrumbs:
        breadcrumb_element = breadcrumb.text
        breadcrumbs_list.append(breadcrumb_element)
    del breadcrumbs_list[0:2]
    del breadcrumbs_list[-1]
    ...
    csv_writer.writerow(['Категория', 'Наименование', 'Цена', 'Описание', 'фото-1', 'фото-2', 'фото-3', 'фото-4'])

    #Если у товара 4 фото, то записываю их, если одно, то одно, если ни одного, то не пишу ссылки на фото
    try:
        csv_writer.writerow([breadcrumbs_list, product_name, product_price, product_descr_formated, foto_list[0], foto_list[1], foto_list[2], foto_list[3]])
    except IndexError:
        try:
            csv_writer.writerow([breadcrumbs_list, product_name, product_price, product_descr_formated, foto_list[0]])
        except IndexError:
            csv_writer.writerow([breadcrumbs_list, product_name, product_price, product_descr_formated])
            pass
        pass
    csv_export.close()
